...
def main():
    ...
    if cloud is not None:
        no_users = int(cloud.get("iam_users", -1)) == 0
        no_root_keys = int(cloud.get("root_access_keys", 1)) == 0
        live.append((
            ...
            "this account is federated and short-lived — and this is read from the account, not "
            "inferred from the code that creates none.",
            {"ok": no_users and no_root_keys, "at": cloud.get("read_at"), "url": cloud.get("_run"),
             "detail": f'{cloud.get("iam_users")} IAM users · root access keys: '
                       f'{"none" if no_root_keys else "present"}',
             "sha": None, "event": None},
            24 * 8))
